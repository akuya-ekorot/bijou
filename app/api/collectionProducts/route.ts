...
export async function PUT(req: Request) {
  try {
    ...
    const { collectionProduct } = await updateCollectionProduct(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = collectionProductIdSchema.parse({ id });
    const { collectionProduct } = await deleteCollectionProduct(validatedParams.id);
