...
export const createShop = async (shop: NewShopParams) => {
  ...
  const newShop = insertShopSchema.parse({ ...shop, userId: session?.user.id! });
  try {
...
export const updateShop = async (id: ShopId, shop: UpdateShopParams) => {
  ...
  const newShop = updateShopSchema.parse({ ...shop, userId: session?.user.id! });
  try {
    const [s] =  await db
     .update(shops)
...
export const deleteShop = async (id: ShopId) => {
  ...
  const { id: shopId } = shopIdSchema.parse({ id });
  try {
    const [s] =  await db.delete(shops).where(and(eq(shops.id, shopId!), eq(shops.userId, session?.user.id!)))
    .returning();
    return { shop: s };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
