...
export const createCollectionImage = async (collectionImage: NewCollectionImageParams) => {
  ...
  const newCollectionImage = insertCollectionImageSchema.parse({ ...collectionImage, userId: session?.user.id! });
  try {
...
export const updateCollectionImage = async (id: CollectionImageId, collectionImage: UpdateCollectionImageParams) => {
  ...
  const newCollectionImage = updateCollectionImageSchema.parse({ ...collectionImage, userId: session?.user.id! });
  try {
    const [c] =  await db
     .update(collectionImages)
     .set({...newCollectionImage, updatedAt: new Date() })
     .where(and(eq(collectionImages.id, collectionImageId!), eq(collectionImages.userId, session?.user.id!)))
     .returning();
    return { collectionImage: c };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
...
export const deleteCollectionImage = async (id: CollectionImageId) => {
  ...
  const { id: collectionImageId } = collectionImageIdSchema.parse({ id });
  try {
    const [c] =  await db.delete(collectionImages).where(and(eq(collectionImages.id, collectionImageId!), eq(collectionImages.userId, session?.user.id!)))
    .returning();
    return { collectionImage: c };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
