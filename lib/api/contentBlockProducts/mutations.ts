...
export const createContentBlockProduct = async (
  contentBlockProduct: NewContentBlockProductParams,
) => {
  const { session } = await getUserAuth();
  const newContentBlockProduct = insertContentBlockProductSchema.parse({
    ...contentBlockProduct,
    userId: session?.user.id!,
  });
  try {
    const [c] = await db
      .insert(contentBlockProducts)
      .values(newContentBlockProduct)
      .returning();
...
export const updateContentBlockProduct = async (
  id: ContentBlockProductId,
  contentBlockProduct: UpdateContentBlockProductParams,
) => {
  const { session } = await getUserAuth();
  const { id: contentBlockProductId } = contentBlockProductIdSchema.parse({
    id,
  });
  const newContentBlockProduct = updateContentBlockProductSchema.parse({
    ...contentBlockProduct,
    userId: session?.user.id!,
  });
  try {
