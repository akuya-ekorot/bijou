...
export const createCollection = async (collection: NewCollectionParams) => {
  ...
  const newCollection = insertCollectionSchema.parse({ ...collection, userId: session?.user.id! });
  try {
...
export const updateCollection = async (id: CollectionId, collection: UpdateCollectionParams) => {
  ...
  const newCollection = updateCollectionSchema.parse({ ...collection, userId: session?.user.id! });
  try {
    const [c] =  await db
     .update(collections)
...
export const deleteCollection = async (id: CollectionId) => {
  ...
  const { id: collectionId } = collectionIdSchema.parse({ id });
  try {
    const [c] =  await db.delete(collections).where(and(eq(collections.id, collectionId!), eq(collections.userId, session?.user.id!)))
    .returning();
    return { collection: c };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
