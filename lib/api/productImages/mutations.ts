...
export const createProductImage = async (productImage: NewProductImageParams) => {
  const newProductImage = insertProductImageSchema.parse(productImage);
  try {
    const [p] =  await db.insert(productImages).values(newProductImage).returning();
...
export const updateProductImage = async (id: ProductImageId, productImage: UpdateProductImageParams) => {
...
export const deleteProductImage = async (id: ProductImageId) => {
  const { id: productImageId } = productImageIdSchema.parse({ id });
  try {
    const [p] =  await db.delete(productImages).where(eq(productImages.id, productImageId!))
    .returning();
    return { productImage: p };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
